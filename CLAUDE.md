...
- `GET /api/v1/health` — Liveness probe (no auth required). Returns **200** if `systemHealth.status` is `ok` or `warn`; **503** if `degraded` (ledger DLQ backlog; brain circuit open is `warn` not `degraded`). Normal body: `{"status":"ok|warn|degraded","service":"ch1tty","systemHealth":{...}}`. **warn body** additionally includes `brainCircuitOpen: true` when `systemHealth.brainDegraded === true` (Ollama/embedding circuit open) and `ledgerWarn: true` when `systemHealth.ledgerStatus === 'warn'` (drops/flushErrors present but no DLQ backlog); either or both may appear together; both absent on `ok`. **503 body** additionally includes `ledgerDlq: { entryCount: N }` so callers can see the backlog depth without a separate `/api/v1/status` call. On internal snapshot failure: **503** with `{"status":"degraded","service":"ch1tty","error":"internal"}` (no `systemHealth` or `ledgerDlq` fields).
