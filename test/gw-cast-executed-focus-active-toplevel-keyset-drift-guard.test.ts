/**
 * GW drift guard: freeze cast:executed exact top-level key set when a focus
 * profile is active.
 *
 ...
 *
 * GW-1  cast:executed WITH focus active (no catalog suggestions) adds exactly
 ...
 *
 * GW-2  cast:executed WITH focus active AND catalog suggestions adds exactly
 ...
 *
 * GW-3  cast:executed WITH focus active does NOT add `scope`,
 ...
 *
 * GW-4  the `focus` value is a non-empty string equal to the active profile
 *        name — not an object, not a boolean, not null.
 *
 * GW-5  `suggestions` when present is an object with exactly the key set
...
test('GW-1: cast:executed with focus active (no catalog suggestions) adds exactly `focus` to base set', async () => {
...
test('GW-2: cast:executed with focus active and catalog suggestions adds exactly `focus` and `suggestions`', async () => {
...
test('GW-3: cast:executed with focus active does NOT contain scope, resolvedFromCatalog, chainContinuation, or explanation', async () => {
  const agg = makeAgg({ withSuggestions: false });
  try {
    const body = await castExecuted(agg);
...
test('GW-4: cast:executed `focus` value is a non-empty string equal to the active profile name', async () => {
  const agg = makeAgg({ withSuggestions: false });
  try {
    const body = await castExecuted(agg);
    assert.ok(
...
test('GW-5: cast:executed `suggestions` object has exactly {combos, prompts} when present', async () => {
  const agg = makeAgg({ withSuggestions: true });
  try {
    const body = await castExecuted(agg);
