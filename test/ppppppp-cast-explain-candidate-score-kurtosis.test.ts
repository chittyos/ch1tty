/**
 * PPPPPPP: explanation.candidateScoreKurtosis in ch1tty/cast when explain:true.
 *
 ...
 *
 * Covered:
 *   PPPPPPP-1: present when >= 2 candidates with different scores
 *   PPPPPPP-2: is a finite number
 *   PPPPPPP-3: absent when only 1 candidate
 *   PPPPPPP-4: absent when all candidates score identically (stddev === 0)
 *   PPPPPPP-5: absent on no_match (zero candidates)
 *   PPPPPPP-6: consistent with variance identity (K = fourth_central_moment / variance^2 − 3)
 *   PPPPPPP-7: present without focus active
 *   PPPPPPP-8: tool description documents candidateScoreKurtosis
 */
...
// Identical score to STRONG (same keywords)
...
test('PPPPPPP-1: candidateScoreKurtosis present when >= 2 candidates with different scores', async () => {
  const agg = buildAgg('n1', [SERVER_A, SERVER_B], {
...
test('PPPPPPP-2: candidateScoreKurtosis is a finite number', async () => {
  const agg = buildAgg('n2', [SERVER_A, SERVER_B, SERVER_C], {
    'server-a': TOOLS_STRONG,
...
test('PPPPPPP-3: candidateScoreKurtosis absent when only 1 candidate', async () => {
  const agg = buildAgg('n3', [SERVER_A], { 'server-a': TOOLS_STRONG });
  try {
    const r = await agg.callTool('ch1tty/cast', { intent: 'billing invoice payment charge create', explain: true });
    const parsed = JSON.parse((r.content[0] as { text: string }).text);
    const { explanation } = parsed;
...
test('PPPPPPP-4: candidateScoreKurtosis absent when all candidates score identically', async () => {
  // Both tools have identical descriptions → same score → variance === 0 → kurtosis undefined
  const agg = buildAgg('n4', [SERVER_A, SERVER_B], {
    'server-a': TOOLS_STRONG,
  ...
  try {
    ...
    const parsed = JSON.parse((r.content[0] as { text: string }).text);
    const { explanation } = parsed;
    if (explanation.candidateScoreVariance !== undefined && explanation.candidateScoreVariance < 1e-9) {
      assert.ok(
        !('candidateScoreKurtosis' in explanation),
        `candidateScoreKurtosis should be absent when variance≈0; found: ${explanation.candidateScoreKurtosis}`,
      );
    }
    // If scores differ due to tokenisation edge cases, kurtosis may be present — that's valid
  } finally {
    await agg.shutdown();
  }
});

test('PPPPPPP-5: candidateScoreKurtosis absent on no_match', async () => {
  const agg = buildAgg('n5', [SERVER_A], {
    'server-a': [{
      name: 'unrelated_tool',
  ...
  try {
    ...
    if (parsed.status === 'no_match') {
      ...
      assert.ok(
        !('candidateScoreKurtosis' in expl),
...
test('PPPPPPP-6: identity K = fourth_central_moment / variance^2 - 3', async () => {
  const agg = buildAgg('n6', [SERVER_A, SERVER_B, SERVER_C], {
    'server-a': TOOLS_STRONG,
    'server-b': TOOLS_MODERATE,
    'server-c': TOOLS_WEAK,
  });
...
test('PPPPPPP-7: candidateScoreKurtosis present without focus active', async () => {
  const path = dlqPath('n7');
...
test('PPPPPPP-8: tool description documents candidateScoreKurtosis', async () => {
