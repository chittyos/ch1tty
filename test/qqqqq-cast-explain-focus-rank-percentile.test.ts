/**
 * QQQQQ: explanation.focusRankPercentile in ch1tty/cast when explain:true and focus is active.
 *
 ...
 *
 * Covered:
 *   QQQQQ-1: focusRankPercentile present when focus active + winner exists
 *   QQQQQ-2: focusRankPercentile === focusRank / candidateCount (mathematical identity)
 *   QQQQQ-3: focusRankPercentile in range (0, 1] always when present
 *   QQQQQ-4: absent on cast:no_match (no winner)
 *   QQQQQ-5: absent when no focus profile is active
 *   QQQQQ-6: equals 1/candidateCount when focusRank === 1 (winner already led pre-focus)
 *   QQQQQ-7: equals 1 when winner was the bottom candidate pre-focus (focusRank === candidateCount)
 *   QQQQQ-8: tool description documents focusRankPercentile
 */
...
test('QQQQQ-1: focusRankPercentile present when focus active + winner exists', async () => {
  const stripeTools: ToolEntry[] = [
    { name: 'create_invoice', description: 'billing invoice payment charge', inputSchema: { type: 'object', properties: {} } },
  ];
  const agg = buildAgg('q1', [STRIPE_CFG], { stripe: stripeTools },
...
test('QQQQQ-2: focusRankPercentile === focusRank / candidateCount (mathematical identity)', async () => {
  const stripeTools: ToolEntry[] = [
    { name: 'create_invoice', description: 'billing invoice payment charge', inputSchema: { type: 'object', properties: {} } },
  ];
...
test('QQQQQ-3: focusRankPercentile in range (0, 1] always when present', async () => {
  const stripeTools: ToolEntry[] = [
    { name: 'create_invoice', description: 'billing invoice payment charge', inputSchema: { type: 'object', properties: {} } },
  ];
  const neonTools: ToolEntry[] = [
...
test('QQQQQ-4: cast:no_match → focusRankPercentile absent (no winner)', async () => {
  const path = dlqPath('q4');
...
test('QQQQQ-5: no focus active → focusRankPercentile absent', async () => {
  const stripeTools: ToolEntry[] = [
    { name: 'create_invoice', description: 'billing invoice payment', inputSchema: { type: 'object', properties: {} } },
  ];
  const agg = buildAgg('q5', [STRIPE_CFG], { stripe: stripeTools }); // no focus
  try {
...
test('QQQQQ-6: focusRankPercentile === 1/candidateCount when focusRank === 1 (winner already led pre-focus)', async () => {
  ...
  const agg = buildAgg('q6', [STRIPE_CFG, NEON_CFG], { stripe: stripeTools, neon: neonTools },
    { focus: 'finance', profiles: FINANCE_PROFILES });
  try {
    const r = await agg.callTool('ch1tty/cast', { intent: 'billing invoice payment charge', explain: true });
    const parsed = JSON.parse((r.content[0] as { text: string }).text);
...
test('QQQQQ-7: focusRankPercentile === 1 when winner was the bottom candidate pre-focus (focusRank === candidateCount)', async () => {
...
test('QQQQQ-8: tool description documents focusRankPercentile', async () => {
