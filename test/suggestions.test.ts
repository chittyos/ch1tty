...
describe('suggestions catalog', () => {
  it('loads all 13 focus profiles', () => {
    const catalog = freshCatalog();
    const keys = Object.keys(catalog).sort();
    assert.deepEqual(keys, ['chittyevidence', 'code', 'communication', 'design', 'finance', 'google', 'governance', 'ledger', 'legal', 'ops', 'realestate', 'session', 'tasks']);
  });
