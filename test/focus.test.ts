...
test('repo focus-profiles.json loads and defines finance/governance/design/code/communication/ops/tasks/ledger/session/chittyevidence', () => {  const profiles = loadFocusProfilesFromPath(join(import.meta.dirname, '..', 'focus-profiles.json'));
  ...
  assert.ok(profiles.profiles.ops, 'ops focus profile present');
  assert.ok(profiles.profiles.tasks, 'tasks focus profile present');
  assert.ok(profiles.profiles.tasks.servers.includes('tasks'), 'tasks profile includes tasks server');
  assert.ok(profiles.profiles.tasks.categories.includes('ecosystem'), 'tasks profile covers ecosystem category');
  assert.strictEqual(profiles.profiles.tasks.boost, 0.6, 'tasks boost is 0.6');
  assert.ok(profiles.profiles.ledger, 'ledger focus profile present');
  ...
  assert.ok(profiles.profiles.chittyevidence, 'chittyevidence focus profile present');
  assert.ok(profiles.profiles.chittyevidence.servers.includes('chittyevidence'), 'chittyevidence profile includes chittyevidence server');
  assert.ok(profiles.profiles.chittyevidence.categories.includes('ecosystem'), 'chittyevidence profile covers ecosystem category');
  assert.strictEqual(profiles.profiles.chittyevidence.boost, 0.6, 'chittyevidence boost is 0.6');
  ...
  assert.ok(profiles.profiles.code.servers.includes('linear'), 'code profile includes linear (issue tracking)');
  assert.ok(profiles.profiles.governance.servers.includes('linear'), 'governance profile includes linear (sprint/project governance)');
  ...
  assert.ok(profiles.profiles.ops.servers.includes('cloudflare'), 'ops profile includes cloudflare');
  assert.ok(profiles.profiles.ops.servers.includes('neon'), 'ops profile includes neon');
  assert.ok(profiles.profiles.ops.servers.includes('github'), 'ops profile includes github');
  assert.ok(profiles.profiles.ops.servers.includes('orchestrator'), 'ops profile includes orchestrator');
});
