...
- **Branch/PR**: `auto/2026-08-21-run-log-1188` → PR #1132
...
- **Guardrail violations**: 261 `auto/*-cast-explain-*-ratio` branches remain on origin. Human cleanup (preview first, then delete): `git for-each-ref --format='%(refname:strip=3)' 'refs/remotes/origin/auto/*-cast-explain-*-ratio' | tee /tmp/guardrail-branches.txt` — review `/tmp/guardrail-branches.txt`, then `cat /tmp/guardrail-branches.txt | xargs git push origin --delete`
- **Notion board**: Notion MCP server connects but API calls fail with HTTP 401 (token expired). `docs/run-log.md` is the fallback log. Fix: refresh the Notion integration token in workspace settings or run `chitty-mcp-token notion`.
