// Ported from src/config.ts — the structural validation (validateServersConfig,
// validateServerConfig, the assert* helpers) is kept verbatim. Dropped:
//  - readFileSync / resolveConfigPath / loadConfigFromPath (no Worker filesystem)
//  - interpolatePath / ${VAR} expansion (env interpolation now happens at
//    connect time in remote-proxy.ts against the Worker env, not at load time)
// Config is supplied as a bundled object (config-data.ts), validated here.
...
const SERVER_KEYS = new Set([
  'id', 'name', 'type', 'access', 'category', 'command', 'args',
  'endpoint', 'authTokenKey', 'headers', 'envHeaders', 'lazy', 'enabled', 'env',
]);
...
function assertOptionalEnv(value: unknown, fieldPath: string): Record<string, string> | undefined {
  if (value === undefined) return undefined;
  if (!isRecord(value)) throw new Error(`${fieldPath} must be an object of string values`);
  const output: Record<string, string> = {};
  for (const [key, envValue] of Object.entries(value)) {
    if (typeof envValue !== 'string') throw new Error(`${fieldPath}.${key} must be a string`);
    output[key] = envValue;
  }
  return output;
}
...
function validateServerConfig(raw: unknown, index: number): ServerConfig {
  ...
  for (const key of Object.keys(raw)) {
    if (!SERVER_KEYS.has(key)) throw new Error(`${prefix}.${key} is not a recognized field`);
  }
  ...
  if (type === 'local') {
    if (!command) throw new Error(`${prefix}.command is required for local servers`);
    return {
      id, name, type,
      access: access as ServerAccess,
      category: category as ServerCategory,
      command, args, lazy, enabled, env,
    };
  }

  if (!endpoint) throw new Error(`${prefix}.endpoint is required for remote servers`);
  return {
    id, name, type,
    access: access as ServerAccess,
    category: category as ServerCategory,
    endpoint, authTokenKey, headers, envHeaders, lazy, enabled,
  };
}
...
export function validateServersConfig(raw: unknown): ServersConfig {
  ...
  const { servers } = raw;
  if (!Array.isArray(servers)) throw new Error('Config root must include a "servers" array');
  ...
  const validated = indexed.map(({ entry, originalIndex }) => validateServerConfig(entry, originalIndex));
  const seen = new Set<string>();
  for (const server of validated) {
    if (seen.has(server.id)) throw new Error(`Duplicate server id "${server.id}" found in config`);
    seen.add(server.id);
  }
