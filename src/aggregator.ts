    ...
    return [
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            sessionId: { type: 'string', description: 'Explicit session ID. When provided, overrides the transport-derived session ID — enabling stateless HTTP server-to-server callers to participate in session tracking (sticky focus, affinity, topTools). Ignored when the transport already supplies a session ID and no override is needed.' },
          },
        },
      },
      {
