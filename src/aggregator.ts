...
function buildCastExplanation(
  ...
  return {
    ...
    ...(focusName && focus ? {
      ...
      focusBoost,
      winnerInFocus,
      ...(best !== undefined ? { winnerFocusBoost: winnerInFocus ? focusBoost : 0, winnerScoreBase: best.score - (winnerInFocus ? focusBoost : 0), candidatesInFocusCount: scoredTools.filter((t) => isInFocus(focus, t)).length, ...(focusRank !== undefined ? { focusRank, focusRankDelta: focusRank - 1 } : {}), ...(unfocusedWinner !== undefined ? { unfocusedWinner } : {}) } : {}),
