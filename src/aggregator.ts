    ...
    return [
      ...
      },
      {
        ...
        description:
          ...
          'explanation also includes runnerUpInFocus: boolean — whether the runner-up tool (the second-highest scored candidate) falls within the active focus profile. Present when a focus profile is active and a runner-up exists (same conditions as focusDecisive and focusMargin). Absent when no focus is active, on no_match, or when there is only one candidate (no runner-up). A value of true means the focus profile claimed both the winner and the runner-up — both leading candidates were boosted. A value of false means the runner-up was out-of-focus — the winner beat an unfocused candidate. Combined with winnerInFocus, lets operators categorize the selection into four quadrants: (true/true) both boosted; (true/false) winner boosted, runner-up not; (false/true) runner-up boosted, winner out-of-focus; (false/false) neither in focus (focus had no direct influence on the top two). ' +
