...
function buildCastExplanation(
  ...
  return {
    ...
    ...(topCandidates.length > 1 ? { runnerUpScore: topCandidates[1].score, runnerUpTool: topCandidates[1].tool, runnerUpServer: topCandidates[1].tool.split('/')[0], runnerUpCategory: scoredTools[1].category, candidateScoreSpread: scoredTools[0].score - scoredTools[scoredTools.length - 1].score, lowestCandidateScore: scoredTools[scoredTools.length - 1].score, ...(candidateScoreEntropy !== undefined ? { candidateScoreEntropy } : {}), ...(topCandidates[1].score > 0 ? { winnerScoreRatio: topCandidates[0].score / topCandidates[1].score } : {}), topCandidatesScoreVariance: (() => { const mean = topCandidates.reduce((s, c) => s + c.score, 0) / topCandidates.length; return topCandidates.reduce((s, c) => s + (c.score - mean) ** 2, 0) / topCandidates.length; })(), topCandidatesScoreStdDev: (() => { const mean = topCandidates.reduce((s, c) => s + c.score, 0) / topCandidates.length; return Math.sqrt(topCandidates.reduce((s, c) => s + (c.score - mean) ** 2, 0) / topCandidates.length); })(), ...(topCandidatesGiniCoefficient !== undefined ? { topCandidatesGiniCoefficient } : {}) } : {}),
    ...(focusName && focus ? {
      focus: focusName,
      focusBoost,
      winnerInFocus,
