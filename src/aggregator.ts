...
function buildCastExplanation(
  ...
  return {
    ...
    ...(topCandidates.length > 1 ? { runnerUpScore: topCandidates[1].score, runnerUpTool: topCandidates[1].tool, runnerUpServer: topCandidates[1].tool.split('/')[0], runnerUpCategory: scoredTools[1].category, candidateScoreSpread: scoredTools[0].score - scoredTools[scoredTools.length - 1].score, ...(topCandidates[1].score > 0 ? { winnerScoreRatio: topCandidates[0].score / topCandidates[1].score } : {}) } : {}),
    ...(focusName && focus ? {
      focus: focusName,
      focusBoost,
      winnerInFocus,
