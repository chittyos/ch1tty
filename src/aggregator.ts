    ...
    return [
      ...
      },
      {
        ...
        description:
          ...
          'explanation also includes candidateScoreDecileRatio: number — the 90:10 decile ratio of the candidate score distribution: candidateScoreP90 / candidateScoreP10. A classic, dimensionless inequality/dispersion measure widely used in economics (where P90/P10 of income is the standard inequality statistic). Present when >= 2 candidates exist and candidateScoreP10 > 0 (division guard; absent when P10 is zero, meaning the 10th percentile is at the zero floor). Absent on cast:no_match, single candidate, or when candidateScoreP10 === 0. Always >= 1: P90 is always >= P10 by definition. Equal to 1 when all candidates score identically (P90 === P10). A value of 2 means the candidate at the 90th percentile scores twice as much as the candidate at the 10th percentile. Large values (>= 5) indicate extreme spread between the upper and lower deciles — a highly unequal pool where the top-tier candidates far outscore the bottom tier. Unlike candidateScoreSpread (sensitive to the single best and worst candidates) and candidateScoreIQR (middle 50%), the decile ratio focuses on the 80th-percentile span while being scale-free: it measures how many times more the upper-decile candidate scores relative to the lower-decile candidate. Identity: candidateScoreDecileRatio * candidateScoreP10 === candidateScoreP90 always holds when present. Complements candidateScoreP80Range (the absolute P10-to-P90 gap) by expressing the same span as a ratio rather than a difference. ' +
...
function buildCastExplanation(
  ...
  const candidateScoreDecileRatio: number | undefined =
    candidateScoreP10 !== undefined && candidateScoreP10 > 0
