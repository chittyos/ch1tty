    ...
    return [
      ...
      },
      {
        ...
        description:
          ...
          'explanation also includes runnerUpScoreBase: number — the runner-up tool\'s relevance score before the active focus boost was applied (runnerUpScore - runnerUpFocusBoost). Present when a focus profile is active and a runner-up exists (same conditions as runnerUpFocusBoost). Absent when no focus is active, on no_match, or when fewer than 2 candidates exist. Equal to runnerUpScore when the runner-up is out-of-focus (runnerUpInFocus is false; no boost applied); strictly less than runnerUpScore when the runner-up is in-focus (runnerUpInFocus is true). The identity runnerUpScoreBase + runnerUpFocusBoost = runnerUpScore always holds, giving a complete decomposition of the runner-up\'s score into its raw and focus-boosted components. Symmetric to winnerScoreBase: together they expose the pre-focus base scores for both the winner and runner-up, enabling pure relevance comparisons that strip out focus effects. ' +
