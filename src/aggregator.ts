    ...
    return [
      ...
      },
      {
        ...
        description:
          ...
          'explanation also includes topCandidatesKurtosis: number — the fourth standardised moment (excess kurtosis) of the relevance scores across the topCandidates pool (up to 5 candidates). Computed as (1/n) * sum((x_i - mean)^4 / stddev^4) - 3 where mean and stddev are the pool\'s population mean and standard deviation. Present when >= 2 topCandidates exist and topCandidatesScoreStdDev > 0 (same conditions as topCandidatesScoreSkewness). Absent on cast:no_match, single candidate, or all-equal scores. Excess kurtosis of 0 indicates a normal-like tail shape (mesokurtic); positive values (leptokurtic) indicate heavier tails and a sharper peak relative to a normal distribution — one or two candidates stand well apart from the pack; negative values (platykurtic) indicate lighter tails and a flatter distribution — top candidates are more uniformly spread. Completes the four-moment statistical characterisation of the top pool alongside topCandidatesScoreVariance (2nd moment — spread), topCandidatesScoreStdDev (spread in score units), and topCandidatesScoreSkewness (3rd moment — asymmetry): together they fully describe the shape of the top-5 score distribution. Full-pool parallel: candidateScoreKurtosis covers the same 4th moment over all scored candidates. ' +
