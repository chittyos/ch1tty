    ...
    return [
      ...
      },
      {
        ...
        description:
          ...
          'explanation also includes candidateScoreKurtosis: number — the excess kurtosis (fourth standardised moment) of relevance scores across the entire candidate pool. Computed as (1/n) * sum((x_i - mean)^4) / stddev^4 - 3 over all scored candidates. Present when >= 2 candidates exist and the full-pool stddev > 0 (same conditions as candidateScoreSkewness). Absent on cast:no_match, single candidate, or when all candidate scores are identical. Excess kurtosis of 0 is the normal-distribution baseline (mesokurtic); positive values (leptokurtic) indicate heavier tails and a sharper central peak than normal — one or a few outlier tools score much higher than the bulk while most cluster near the mean; negative values (platykurtic) indicate lighter tails and a flatter distribution — scores are spread more uniformly across the range with fewer extreme outliers. Completes the four-moment statistical characterisation of the full candidate distribution alongside candidateScoreMean (1st moment — center), candidateScoreVariance and candidateScoreStdDev (2nd moment — spread), and candidateScoreSkewness (3rd moment — asymmetry): together they give a complete parametric summary of the score distribution shape without returning all raw scores. The identity candidateScoreKurtosis = ((1/n) * sum((x_i - mean)^4)) / candidateScoreVariance^2 - 3 always holds when present. ' +
...
function buildCastExplanation(
  ...
  // Excess kurtosis (fourth standardised moment) of the full candidate pool; absent when < 2 candidates or stddev === 0.
  const candidateScoreKurtosis: number | undefined = (() => {
    ...
    return scoredTools.reduce((s, t) => s + (t.score - mean) ** 4, 0) / (n * variance ** 2) - 3;
  })();
