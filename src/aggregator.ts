    ...
    return [
      ...
      },
      {
        ...
        description:
          ...
          'explanation also includes focusNetBoostDelta: number — the net differential focus boost the winner received compared to the runner-up (winnerFocusBoost − runnerUpFocusBoost). Present when a focus profile is active and a runner-up exists (same conditions as rawFocusMargin). Absent when no focus is active, on no_match, or fewer than 2 candidates. Always one of three values: +focusBoost (winner is in-focus, runner-up is out-of-focus — focus gave the winner an unmatched advantage); 0 (both tools have the same focus state — either both in-focus or both out-of-focus, so the boost cancelled out); or −focusBoost (winner is out-of-focus, runner-up is in-focus — focus helped the runner-up more). Completes the focus margin decomposition triangle: the identity focusMargin = rawFocusMargin + focusNetBoostDelta always holds (focusMargin is the post-focus gap; rawFocusMargin is the pre-focus gap; focusNetBoostDelta is the net boost differential). When focusNetBoostDelta > 0, focus widened the gap; when 0, both tools were affected equally by focus so the margin is pure raw relevance; when focusNetBoostDelta < 0, focus narrowed the gap. ' +
