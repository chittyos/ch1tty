/**
 * HTTP transport tests for comms-mcp (Workstream W).
 *
 * Spins up a real HTTP server bound to a random port (0) for each fixture,
 * makes real fetch() requests, and checks the responses. Covers /health,
 * bearer-auth guard on /mcp, DNS-rebinding rejection (Origin header), and
...
test('GET /health returns 200 with JSON body', async () => {
  ...
  try {
    ...
    assert.equal(body.service, 'comms-mcp');
  } finally {
    await stop();
  }
});
...
// ── DNS-rebinding protection (CWE-346) ────────────────────────────────────────

test('POST /mcp with Origin header returns 403 (DNS-rebinding protection)', async () => {
  const { baseUrl, stop } = await startApp();
  try {
...
test('/mcp without token configured — any request is allowed through to MCP handler', async () => {
  ...
  try {
    ...
    // Not 401 — auth is not configured
