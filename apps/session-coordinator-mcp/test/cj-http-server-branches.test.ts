/**
 * CJ: session-coordinator-mcp HTTP server URL-variant branch coverage
 *
 * Covers branches in apps/session-coordinator-mcp/src/http-server.ts not
 * reached by the existing http-transport.test.ts:
 *
...
test('session-coordinator-mcp: POST /mcp?foo=bar routes to MCP handler (not 404)', async () => {
...
test('session-coordinator-mcp: POST /mcp/something routes to MCP handler (not 404)', async () => {
  const { baseUrl, stop } = await startApp();
  try {
