/**
 * CJ: ledger-mcp HTTP server URL-variant branch coverage
 *
 * Covers branches in apps/ledger-mcp/src/http-server.ts not reached by
...
test('ledger-mcp: POST /mcp?foo=bar routes to MCP handler (not 404)', async () => {
...
test('ledger-mcp: POST /mcp/something routes to MCP handler (not 404)', async () => {
  const { baseUrl, stop } = await startApp();
  try {
