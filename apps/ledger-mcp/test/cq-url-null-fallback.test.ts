/**
 * CQ: ledger-mcp HTTP server req.url null-coalescing fallback coverage
 *
 * Covers the `const url = req.url ?? '';` branch in
 * apps/ledger-mcp/src/http-server.ts line 38: the right-hand side
...
test('ledger-mcp: req.url undefined → empty-string fallback → 404', async () => {
  const app = createLedgerMcpHttpApp();
