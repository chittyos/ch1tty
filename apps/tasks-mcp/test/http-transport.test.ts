/**
 * HTTP transport tests for tasks-mcp (Workstream R).
 *
...
test('GET /health returns 200 with JSON body', async () => {
  ...
  try {
    ...
    assert.equal(body.service, 'tasks-mcp');
  } finally {
    await stop();
  }
});
...
test('/mcp without token configured — any request is allowed through to MCP handler', async () => {
  const { baseUrl, stop } = await startApp();
  try {
    // POST /mcp with no session ID → MCP session manager returns 400 (no session)
