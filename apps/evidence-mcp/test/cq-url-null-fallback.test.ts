/**
 * CQ: evidence-mcp HTTP server req.url null-coalescing fallback coverage
 *
 * Covers the `const url = req.url ?? '';` branch in
 * apps/evidence-mcp/src/http-server.ts line 38: the right-hand side
...
test('evidence-mcp: req.url undefined → empty-string fallback → 404', async () => {
  const app = createEvidenceHttpApp();
