...
| `comms-mcp/` | implemented — unified cross-channel comms surface; 1 tool (`recentLog`) fusing quo/imessage/email/twilio into one time-ordered message log with self-collapse and per-channel degradation; `enabled: false` until `COMMS_MCP_<CHANNEL>_ENDPOINT` / `_TOKEN` / `_CF_ACCESS_*` env vars set | (local stdio) | Cross-channel communication log |
