...
| `comms-mcp/` | implemented — unified cross-channel comms surface; 1 tool (`recentLog`) fusing quo/imessage/email/twilio into one time-ordered message log with self-collapse and per-channel degradation; activate by setting `"enabled": true` in `servers.json`, then set per-channel env vars: `COMMS_MCP_<SERVERID>_ENDPOINT` (required; missing endpoint fails that channel only), `_TOKEN`, `_CF_ACCESS_CLIENT_ID`, `_CF_ACCESS_CLIENT_SECRET` (optional; fall back to bare `CF_ACCESS_CLIENT_ID`/`_SECRET`) | (local stdio) | Cross-channel communication log |
