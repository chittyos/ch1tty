...
| `evidence-mcp/` | scaffolded — REST adapter wrapping `evidence.chitty.cc`; 5 typed tools (ingest/list/get/search documents + get_canonical_uri); `enabled: false` until `CHITTY_EVIDENCE_TOKEN` set | `evidence.chitty.cc/mcp` (future) | Document ingest + canonical URIs |
| `comms-mcp/` | implemented — unified cross-channel comms surface; 1 tool (`comms.recentLog`) fusing bound providers into one time-ordered message log with self-collapse and per-channel degradation; bound: **quo** (`chittyagent-quo`) + **email** (`chittyagent-google`); imessage/twilio/voice accepted by schema but degrade to `ok:false` (no bound provider); activate by setting `"enabled": true` in `servers.json`, then set env vars keyed by backend MCP server ID: `COMMS_MCP_CHITTYAGENT_QUO_ENDPOINT`, `COMMS_MCP_CHITTYAGENT_GOOGLE_ENDPOINT` (required; missing → that channel degrades only), `_TOKEN`, `_CF_ACCESS_CLIENT_ID`, `_CF_ACCESS_CLIENT_SECRET` (optional; fall back to bare `CF_ACCESS_CLIENT_ID`/`_SECRET`) | (local stdio) | Cross-channel communication log |
