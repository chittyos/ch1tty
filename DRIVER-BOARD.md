...
- [x] **PPPP. `ch1tty/cast` `latencyBreakdown.brainMs`** — When the brain route fires (`castRoute === 'brain'`), `latencyBreakdown` on `cast:executed` and `cast:chain_executed` now includes `brainMs: number` — the wall-clock time for `routeIntent()` alone. Completes the 3-way timing split: `brainMs` (brain routing) + `scoringMs` (total pre-execution) + `executionMs` (backend). Absent when keyword-fallback route taken. PR #456 ✅ MERGED (45bdce9, run 139, 2026-06-14). 7 new tests, 1291/0/2. DONE.
...
- OOOO (#455) ✅ MERGED (3656961). PPPP (#456) ✅ MERGED (45bdce9).
