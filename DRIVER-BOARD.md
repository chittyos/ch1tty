...
**Opened:** PR #1407 (FD — keyword search response envelope + tool item drift guard)
- Branch: `auto/FD-search-keyword-response-drift-guard`
- File: `test/fd-search-keyword-response-drift-guard.test.ts` (12 tests, 4 suites)
...
- 4 suites:
  ...
  2. Top-level conditional keys: focus → `+focus`; inFocusOnly → `+inFocusOnly` (2 tests)
...
**Workstream status:**
  ...
  - [ ] **FD** — PR #1407 open (CI pending).

**Human-action items (unchanged):**
  1. **DISABLE hourly cron** — ~1728 runs; burning ~50k tokens/run
  2. **Merge PR #1407 (FD)** once CI green — keyword search envelope + tool item drift guard
  3. **Prod env vars**: GITHUB_MCP_AUTHORIZATION, CHITTY_CF_ACCESS_CLIENT_ID, CHITTY_CF_ACCESS_CLIENT_SECRET
  4. **Enable GitHub Actions** (main npm test CI job — CI still only CodeQL)
  5. **Notion workspace** out of free blocks — upgrade or clear
  6. **Stale branch cleanup** — 1100+ remote auto/ branches
